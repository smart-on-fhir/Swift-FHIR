...
//  SMART-on-FHIR
//
//  Generated from FHIR 0.4.0.4394 (http://hl7.org/fhir/StructureDefinition/Practitioner) on 2015-03-11.
...
public class Practitioner: DomainResource
{
	...
	public required init(json: JSONDictionary?) {
		...
		if let js = json {
			...
			if let val = js["practitionerRole"] as? [JSONDictionary] {
...
public class PractitionerPractitionerRole: FHIRElement
{
	...
	public required init(json: JSONDictionary?) {
		super.init(json: json)
		if let js = json {
			if let val = js["healthcareService"] as? [JSONDictionary] {
