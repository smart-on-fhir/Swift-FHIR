...
//  SwiftFHIR
//
//  Generated from FHIR 3.3.0.13671 (http://hl7.org/fhir/StructureDefinition/Invoice) on 2018-05-03.
...
Each line item represents one charge for goods ond services rendered. Details such as date, code and amount are found in
...
open class InvoiceLineItem: BackboneElement {
	...
	public var chargeItem: Reference?
	...
	/** Convenience initializer, taking all required properties as arguments. */
	public convenience init(chargeItem: Reference) {
		self.init()
	...
	override open func populate(from json: FHIRJSON, context instCtx: inout FHIRInstantiationContext) {
		...
		chargeItem = createInstance(type: Reference.self, for: "chargeItem", in: json, context: &instCtx, owner: self) ?? chargeItem
		if nil == chargeItem && !instCtx.containsKey("chargeItem") {
			instCtx.addError(FHIRValidationError(missing: "chargeItem"))
		}
	...
	override open func decorate(json: inout FHIRJSON, errors: inout [FHIRValidationError]) {
		...
		self.chargeItem?.decorate(json: &json, withKey: "chargeItem", errors: &errors)
		if nil == self.chargeItem {
			errors.append(FHIRValidationError(missing: "chargeItem"))
		}
...
is currently under developement. The priceComponent element can be used to offer transparency to the recipient of the
...
open class InvoiceLineItemPriceComponent: BackboneElement {
	...
	override open func populate(from json: FHIRJSON, context instCtx: inout FHIRInstantiationContext) {
		...
		factor = createInstance(type: Money.self, for: "factor", in: json, context: &instCtx, owner: self) ?? factor
