...
//  SwiftFHIR
//
//  Generated from FHIR 1.7.0.10073 (http://hl7.org/fhir/StructureDefinition/ServiceDefinition) on 2016-10-26.
...
open class ServiceDefinition: DomainResource {
	...
	override open func populate(from json: FHIRJSON?, presentKeys: inout Set<String>) -> [FHIRJSONError]? {
		...
		if let js = json {
			...
			if let exist = js["operationDefinition"] {
				presentKeys.insert("operationDefinition")
				if let val = exist as? FHIRJSON {
					self.operationDefinition = Reference(json: val, owner: self)
				}
				else {
					errors.append(FHIRJSONError(key: "operationDefinition", wants: FHIRJSON.self, has: type(of: exist)))
				}
			}
	...
	override open func asJSON() -> FHIRJSON {
		...
		if let operationDefinition = self.operationDefinition {
			json["operationDefinition"] = operationDefinition.asJSON()
		}
