...
//		XCTAssertEqual(inst.useContext![0].coding![3].system!.absoluteString!, "http://unknown/UAMS Clinical Research")
	...
	func testDataElement6() {
		let instance = testDataElement6_impl()
		testDataElement6_impl(json: instance.asJSON())
	}
	
	func testDataElement6_impl(json: JSONDictionary? = nil) -> DataElement {
		let inst = (nil != json) ? instantiateFrom(json: json!) : instantiateFrom(filename: "dataelement-sdc-profile-example.canonical.json")
	...
	func testDataElement7() {
		let instance = testDataElement7_impl()
		testDataElement7_impl(json: instance.asJSON())
	}
	
	func testDataElement7_impl(json: JSONDictionary? = nil) -> DataElement {
