...
{
	func instantiateFrom(filename filename: String) throws -> MedicationDispense {
		return instantiateFrom(json: try readJSONFile(filename)!)
	}
	
	func instantiateFrom(json json: FHIRJSON) -> MedicationDispense {
	...
	func runMedicationDispense1(json: FHIRJSON? = nil) throws -> MedicationDispense {
		...
		XCTAssertEqual(inst.dosageInstruction![0].scheduleTiming!.repeat_fhir!.frequency!, 3)
		XCTAssertEqual(inst.dosageInstruction![0].scheduleTiming!.repeat_fhir!.period!, NSDecimalNumber(string: "1"))
		XCTAssertEqual(inst.dosageInstruction![0].scheduleTiming!.repeat_fhir!.periodUnits!, "d")
