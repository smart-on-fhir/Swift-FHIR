...
class ClaimTests: XCTestCase {
	...
	func runClaim1(_ json: FHIRJSON? = nil) throws -> SwiftFHIR.Claim {
		...
		XCTAssertTrue(inst.insurance?[0].focal ?? false)
		XCTAssertEqual(inst.insurance?[0].sequence, UInt(1))
	...
	func runClaim2(_ json: FHIRJSON? = nil) throws -> SwiftFHIR.Claim {
		...
		XCTAssertEqual(inst.item?[1].careTeamLinkId?[0], UInt(1))
		...
		XCTAssertEqual(inst.item?[2].careTeamLinkId?[0], UInt(1))
