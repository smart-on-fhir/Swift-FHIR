...
class DecisionSupportServiceModuleTests: XCTestCase {
	...
	func instantiateFrom(filename filename: String) throws -> DecisionSupportServiceModule {
		return instantiateFrom(json: try readJSONFile(filename))
	}
	
	func instantiateFrom(json json: FHIRJSON) -> DecisionSupportServiceModule {
		let instance = DecisionSupportServiceModule(json: json)
