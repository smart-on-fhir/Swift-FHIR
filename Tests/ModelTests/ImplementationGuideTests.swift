...
class ImplementationGuideTests: XCTestCase {
	...
	func runImplementationGuide1(_ json: FHIRJSON? = nil) throws -> SwiftFHIRImplementationGuide {
		...
		XCTAssertEqual(inst.definition?.package?[0].description_fhir, "Base package (not broken up into multiple packages)")
		XCTAssertEqual(inst.definition?.package?[0].name, "test")
		...
		XCTAssertEqual(inst.definition?.parameter?[0].code, GuideParameterCode(rawValue: "apply-business-version")!)
		XCTAssertEqual(inst.definition?.parameter?[0].value, "1.0")
		XCTAssertEqual(inst.definition?.resource?[0].description_fhir, "A test example to show how a implementation guide works")
