...
class ProtocolTests: XCTestCase {
	...
	func instantiateFrom(filename filename: String) throws -> Protocol {
		return instantiateFrom(json: try readJSONFile(filename))
	}
	
	func instantiateFrom(json json: FHIRJSON) -> Protocol {
		let instance = Protocol(json: json)
	...
	func testProtocol1() {
		do {
			let instance = try runProtocol1()
			try runProtocol1(instance.asJSON())
		}
		catch {
			XCTAssertTrue(false, "Must instantiate and test Protocol successfully, but threw")
		}
	}
	
	func runProtocol1(json: FHIRJSON? = nil) throws -> Protocol {
		...
		XCTAssertTrue(inst.step![0].precondition!.union![0].condition!.valueBoolean!)
		XCTAssertEqual(inst.step![0].precondition!.union![1].condition!.type!.text!, "BMI in last 2 years")
		XCTAssertFalse(inst.step![0].precondition!.union![1].condition!.valueBoolean!)
	...
	func testProtocol2() {
		do {
			let instance = try runProtocol2()
			try runProtocol2(instance.asJSON())
		}
		catch {
			XCTAssertTrue(false, "Must instantiate and test Protocol successfully, but threw")
		}
	...
	func runProtocol2(json: FHIRJSON? = nil) throws -> Protocol {
