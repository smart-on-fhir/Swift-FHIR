...
class SequenceTests: XCTestCase {
	...
	func runSequence9(_ json: FHIRJSON? = nil) throws -> SwiftFHIR.Sequence {
		...
		XCTAssertEqual(inst.pointer?[0].reference, "Sequence/graphic-example-2")
	...
	func runSequence10(_ json: FHIRJSON? = nil) throws -> SwiftFHIR.Sequence {
		...
		XCTAssertEqual(inst.coordinateSystem, 0)
		XCTAssertEqual(inst.id, "graphic-example-2")
		XCTAssertEqual(inst.pointer?[0].reference, "Sequence/graphic-example-3")
		XCTAssertEqual(inst.referenceSeq?.referenceSeqString, "CGCCATTG")
		XCTAssertEqual(inst.referenceSeq?.strand, 1)
		XCTAssertEqual(inst.referenceSeq?.windowEnd, 8)
		XCTAssertEqual(inst.referenceSeq?.windowStart, 0)
		XCTAssertEqual(inst.text?.status, NarrativeStatus(rawValue: "generated")!)
