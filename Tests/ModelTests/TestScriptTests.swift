...
class TestScriptTests: XCTestCase
{
	...
	func instantiateFrom(json json: FHIRJSON) -> TestScript {
		let instance = TestScript(json: json)
	...
	func runTestScript1(json: FHIRJSON? = nil) throws -> TestScript {
		...
		XCTAssertTrue(inst.test![1].metadata!.capability![0].validated!)
		...
		XCTAssertTrue(inst.test![1].metadata!.capability![1].validated!)
		...
		XCTAssertTrue(inst.test![2].metadata!.capability![0].validated!)
	...
	func runTestScript2(json: FHIRJSON? = nil) throws -> TestScript {
		...
		XCTAssertEqual(inst.test![0].action![2].assert!.headerField!, "Last-Modified")
		XCTAssertEqual(inst.test![0].action![2].assert!.operator_fhir!, "notEmpty")
		XCTAssertTrue(inst.test![0].action![2].assert!.warningOnly!)
		XCTAssertEqual(inst.test![0].action![3].assert!.resource!, "Patient")
